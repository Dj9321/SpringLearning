...
public class PropertyBasedNamingStrategy implements PhysicalNamingStrategy {
    ...
    private static final Properties tableMappings = new Properties();

    static {
        try {
            tableMappings.load(
                PropertyBasedNamingStrategy.class.getClassLoader()
                        .getResourceAsStream("application.properties") //table-mapping
            );
        } catch (IOException e) {
    ...
    public Identifier toPhysicalTableName(Identifier name, JdbcEnvironment context) {
        // Hibernate uses entity simple class name as default if no @Table(name=...) provided
        String entityName = name.getText();
        String mappedName = tableMappings.getProperty(entityName, entityName);
